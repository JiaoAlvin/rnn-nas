...
class SearchDelegator:
    ...
    def search(self):
        ...
        try:
            self.evaluate_objective_functions(list(self.architectures.keys()))
        ...
        if self.config('number_of_generations') > 0:
            ...
            for i in range(self.config('number_of_generations')):
                ...
                try:
                    self.evaluate_objective_functions(self.children)
                except TimeExceededException:
                    SlackPost.post_neutral('Time Exceeded', 'Time exceeded, returning.')
                    self.finalise_search()
                    return
    ...
    def initialise_population(self, population_size: int, transformer: BlockTransformation):
        ...
        LOG.info(f'Population size = {population_size}, current architectures = {len(self.architectures)}.')
        if len(self.architectures.keys()) == population_size:
            return

        population_size -= len(self.architectures.keys())
    ...
    def non_dominated_sorting(self):
        with NonDominatedSorting(self.population_fitness, self.config('objectives')) as sorter:
            self.fronts = sorter.sort(self.architectures.keys())
    ...
    def evaluate_objective_functions(self, population: list):
        ...
        for key in population:
            ...
            number_of_parameters, number_of_blocks, number_of_mul, _ = \
                ObjectiveEvaluation.evaluate_cheap_objectives(architecture, key)
            self.population_fitness[key].number_of_functions = number_of_blocks
    ...
    def restore_snapshot(self):
        ...
        if len(architectures_to_evaluate) > 0:
            SlackPost.post_neutral('Evaluating restored',
                                   f'Evaluating architectures without fitness : {architectures_to_evaluate}')
    ...
    def check_times(self):
        ...
        if len(self.generation_times) > 0:
            ...
            SlackPost.post_neutral('Average generation time',
                                   f'Current average generation training time is {avg_time // 60} minutes.')
            diff = time.time() - self.search_start_time
    ...
    def log_current_metrics(self):
        ...
        for key in self.architectures.keys():
            if key in self.population_fitness.keys():
                ...
                total_number_of_functions += model_fitness.number_of_functions
        ...
        TensorBoardWriter.write_scalar('search', f'Search/average/total_number_of_functions',
                                       total_number_of_functions, self.generation)
